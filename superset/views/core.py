...
class Superset(BaseSupersetView):  # pylint: disable=too-many-public-methods
    ...
    def explore_json(
        ...
        if (
            response_type == utils.ChartDataResultFormat.EXCEL
            and not security_manager.can_access("can_excel", "Superset")
        ):
            return json_error_response(
                _("You don't have the rights to ") + _("download as excel"), status=403,
            )
    ...
    def dashboard(  # pylint: disable=too-many-locals
        ...
        datasource_slices = utils.indexed(dashboard.slices, "datasource")
        data = {
        ...
        superset_can_excel = security_manager.can_access("can_csv", "Superset")
